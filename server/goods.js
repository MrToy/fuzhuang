...
router.post('/',async ctx=>{
	...
	var {name,info,imgs,price,cate,itemCode,colors,sizes,remain}=await parse.json(ctx)
	var shop=await ctx.mongo.collection("shops").findOne({owner:_id})
	if(!shop)
		throw "还未创建店铺"
	await ctx.mongo.collection("goods").insert({shop:shop._id,onSale:false,createTime:new Date(),name,info,imgs,price,cate,itemCode,colors,sizes,remain})
...
router.put('/:id',async ctx=>{
	var {_id}=await getUser(ctx)
	var {name,info,imgs,price,cate,itemCode,colors,sizes,onSale,remain}=await parse.json(ctx)
	var id=ObjectID(ctx.params.id)
	var shop=await ctx.mongo.collection("shops").findOne({owner:_id})
	await ctx.mongo.collection("goods").updateOne({"_id":id,shop:shop._id},{"$set":{name,info,imgs,price,cate,itemCode,colors,sizes,onSale,remain}})
	ctx.body={success:true}
})
