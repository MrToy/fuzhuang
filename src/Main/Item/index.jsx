	...
	render(){
		return (
			<div style={{marginRight:20,display:'inline-block',verticalAlign:"top"}}>
				<img style={{width:350,height:350,display:"block"}} src={this.props.imgs[this.state.selected]?("/files/resize?path="+this.props.imgs[this.state.selected].path+"&w=400&h=400"):require('./014-image.png')} />
				{this.props.imgs.slice(0,4).map((it,i)=>(
					<img onClick={()=>this.setState({selected:i})} style={{width:82,height:82,margin:"15px 5px 0 0",cursor:"pointer",border:"2px solid "+(this.state.selected==i?"#C81624":"transparent"),padding:3}} src={("/files/resize?path="+it.path+"&w=90&h=90")||require('./014-image.png')} />
				))}
			</div>
		)
	}
